  ...
  smearingType_(type)
{
  ...
  std::cout << "Read the scales " << std::endl;
  if(!correctionFileName.empty()) { 
    std::string filename = correctionFileName+"_scales.dat";
    readScalesFromFile(filename);
    if(scales_.empty()) {
  ...
  std::cout << "Read the smearings " << std::endl;
  if(!correctionFileName.empty()) { 
    std::string filename = correctionFileName+"_smearings.dat";
    readSmearingsFromFile(filename);
    if(smearings_.empty()) {
...
float EnergyScaleCorrection::smearingSigma(int runnr, double et, double eta, double r9, unsigned int gainSeed, float nrSigmaRho, float nrSigmaPhi) const
{
