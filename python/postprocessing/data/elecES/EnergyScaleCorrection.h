#ifndef EnergyScaleCorrection_h
#define EnergyScaleCorrection_h
...
public:  
  ...
  public:  
    ...
    float sigma(const float et,const float nrSigmaRho=0.,const float nrSigmaPhi=0.)const{
        // std::cout << "et " << et << "  nRho " << nrSigmaRho << "   nPhi " << nrSigmaPhi << std::endl;
      const float rhoVal = rho_ + rhoErr_ * nrSigmaRho;
        // std::cout << "rhoVal " << rhoVal << "  rho_ " << rho_ << "   rhoErr_ " << rhoErr_ << std::endl;
  ...
  {  
  public:
    CorrectionCategory() { throw int(4); }
    ...
    CorrectionCategory(const unsigned int runMin, const unsigned int runMax,
		...
		gain_(gainSeed){
      std::cout << runMin_ << " " << runMax_ << " " << etaMin_ << std::endl;
	}
